...
class VoiceSession:
    ...
    async def initialize_processors(self):
        ...
        try:
            ...
            logger.error(
                f"Error initializing processors for session {self.session_id}: {e}"
            )
            raise
    ...
    def _convert_part_to_message(
        ...
        try:
            ...
            elif content_type == VoiceMetadata.TRANSCRIPT and part.metadata.get(
                "is_complete"
            ):
                return {
            ...
            # LLM text response
            elif content_type == VoiceMetadata.LLM_TEXT and part.metadata.get(
                "is_complete"
            ):
                return {
                    "type": "llm_text",
            ...
            # TTS audio
            elif content_type == VoiceMetadata.TTS_AUDIO and part.metadata.get(
                "is_complete"
            ):
                ...
                audio_b64 = (
                    base64.b64encode(part.content).decode()
    ...
    async def send_audio_data(self, audio_data: bytes) -> bool:
        ...
        try:
            ...
            async for result in self.whisper_processor.call(
                streams.stream_content([audio_part])
            ):
    ...
    async def cleanup(self):
        ...
        try:
            ...
            # Cleanup processors
            if self.whisper_processor:
                await self.whisper_processor._disconnect()
            if self.kokoro_processor and hasattr(
                self.kokoro_processor, "_cleanup_client"
            ):
                await self.kokoro_processor._cleanup_client()
            if self.session_processor and hasattr(
                self.session_processor, "cleanup_session"
            ):
...
async def websocket_endpoint(websocket: WebSocket, path: str):
    ...
    logger.info(f"WebSocket scope: {websocket.scope}")
        ...
        while True:
            try:
                ...
                if "bytes" in data:
                    ...
                    if not success:
                        logger.warning(
                            f"Failed to process audio for session {session_id}"
                        )
    ...
    try:
        ...
        if msg_type == "interrupt":
            ...
            await session.frontend_ws.send_json(
                {"type": "metrics_response", "metrics": metrics}
            )
...
async def startup_event():
    ...
    logger.info(
        f"Configuration: STT={config.STT_MODEL}, LLM={config.LLM_MODEL}, TTS={config.TTS_VOICE}"
    )
