export class VoiceWebSocket {
  ...
  private onSentenceCallback?: (sentence: string, audioData?: ArrayBuffer) => void;  // NEW: Add audio data parameter
              ...
              message.segments.forEach((segment: any) => {
                if (segment.completed && segment.text && segment.text.trim()) {
                  ...
                  if (!this.processedSegments.has(text)) {
                    ...
                    // NEW: Pass undefined for audioData since we get it separately from recorder
                    this.onSentenceCallback?.(text, undefined);
                  }
                }
              });
  ...
  // NEW: Modified to include audio data parameter
  onSentence(callback: (sentence: string, audioData?: ArrayBuffer) => void) {
