...
export default function VoiceButton({ onStatusChange, onTranscript, onError }: VoiceButtonProps) {
  ...
  const [embeddingAttempts, setEmbeddingAttempts] = useState(0);
  const [speakerRecognized, setSpeakerRecognized] = useState(false);
      ...
      try {
        ...
        whisperWs.onSentence(async (sentence) => {
          if (mounted) {
            ...
            const handleSentenceProcessing = useCallback(async (sentence: string) => {
        ...
        // Embed speaker after initialization
        // if (recorderRef.current) {
        //   console.log("Getting 5-second audio sample for speaker embedding...");
        //   const audioBlob = await recorderRef.current.getFiveSecondAudioSample();
        //   const reader = new FileReader();
